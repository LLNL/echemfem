...
class AdvectionDiffusionPoissonSolver(EchemSolver):
    def __init__(self, N, family="DG"):
        ...
        def f(C):
            ...
            z1 = 2.0
            z2 = -2.0
            f1 = div((self.vel) * C1ex) - div(D1 * grad(C1ex))
            f2 = div((self.vel) * C2ex) - div(D2 * grad(C2ex))
        ...
        physical_params = {"flow": ["diffusion", "advection", "poisson"],
                           "F": 1.0,
                           "R": 1.0,
                           "T": 1.0,
                           "U_app": Uex,
...
def test_convergence():
    ...
    for i in range(5):
        ...
        assert errC1 < 0.29 * errC1_old
        assert errC2 < 0.29 * errC2_old
        assert errU < 0.29 * errU_old
        errC1_old = errC1
        errC2_old = errC2
        errU_old = errU
...
def test_convergence_CG():
    ...
    errU_old = 1e6
    for i in range(5):
        solver = AdvectionDiffusionPoissonSolver(2**(i + 1), family="CG")
        solver.setup_solver()
        solver.solve()
        c1, c2, U = solver.u.subfunctions
        errC1 = errornorm(solver.C1ex, c1)
