...
for Vs in Vlist:
    ...
    x_ = solver.mesh.coordinates.vector().dat.data

    i1 = Function(V1).interpolate(-solver.effective_diffusion(sigma_CL, phase="solid") * grad(phi1)[0])

    NCO2 = Function(V1).interpolate( solver.effective_diffusion( D[0]) *
            grad(cCO2)[0])
    NOH = Function(V1).interpolate((solver.effective_diffusion( D[1]) *
            grad(cOH) + F * z[1] * D[1] / R / T * cOH * grad(phi2))[0])
    NH = Function(V1).interpolate((solver.effective_diffusion(D[2]) *
            grad(cH) + F * z[2] * solver.effective_diffusion(D[2]) / R / T * cH
            * grad(phi2))[0])
    NCO3 = Function(V1).interpolate((solver.effective_diffusion(D[3]) *
            grad(cCO3) + F * z[3] * solver.effective_diffusion(D[3]) / R / T *
            cCO3 * grad(phi2))[0])
    NHCO3 = Function(V1).interpolate((solver.effective_diffusion(D[4]) *
            grad(cHCO3) + F * z[4] * solver.effective_diffusion(D[4]) / R / T *
            cHCO3 * grad(phi2))[0])
    NK = Function(V1).interpolate((solver.effective_diffusion(D[5]) *
            grad(cK) + F * z[5] * solver.effective_diffusion(D[5])/ R / T * cK
            * grad(phi2))[0])

    i2 = Function(V1).assign(- F * (z[5] * NK + z[2] * NH + z[4] * NHCO3 +
    ...
    phi1_ = Function(V).interpolate(phi1).vector().dat.data
    phi2_ = Function(V).interpolate(phi2).vector().dat.data
    cK_ = Function(V).interpolate(cK).vector().dat.data
    cH_ = Function(V).interpolate(cH).vector().dat.data
    cHCO3_ = Function(V).interpolate(cHCO3).vector().dat.data
    cOH_ = Function(V).interpolate(cOH).vector().dat.data
    cCO3_ = Function(V).interpolate(cCO3).vector().dat.data
    cCO2_ = Function(V).interpolate(cCO2).vector().dat.data

    i1_ = i1.dat.data
    i2_ = i2.dat.data

    Vname = np.rint(-Vs*1000)
    filename = "gde_data%dmV.csv" % Vname
